...
int main(int, char**)
{
  ...
  //createTrackbar( "background_low", "original", &iLowS, 255, tb_b_low );
	cv::createTrackbar( "cone_high", "original", &iHighS, 255, tb_c_high );
	cv::createTrackbar( "cone_low", "original", &iLowV, 255, tb_c_low );
  ...
  feature_detection_class ftd;

  for (int i=0; i<4; i++){ //pattern_list.size()

    path image_path = pattern_list.at(i);
    std::cout << "loading image " << image_path << std::endl;
    cv::Mat image = cv::imread(image_path, image_mode );

    // to find the size where akaze throws up
    cv::Size target_size(image.cols/8,image.rows/8);
    //cv::resize(image, image, target_size, cv::INTER_LINEAR);

    //cones.push_back(image);



    ftd.extract_keypoints(image);
    cv::Mat result = image.clone();
    //std::cout << "type before keypoints = " << result.type() << std::endl;
    ftd.draw_keypoints(result);
    //std::cout << "type after keypoints = " << result.type() << std::endl;
    //imshow("fuck",result);

    //cv::waitKey(0);
    cones.push_back(result);

    if ( (result.rows > largest_rows) | (result.cols > largest_rows) ){
      largest_rows = result.rows;
      largest_cols = result.cols;
      largest_cone = result.clone();
    }
    ...
    //}
    //std::string iname = "c"+to_string(counter);
    //window_man.set_image(iname,image);
  }

  display_manager window_man(4);
  double ratio = (double)largest_rows/(double)largest_cols;
  int target_rows = 250, target_cols = (int) (ratio * target_rows);
  cv::Mat window_size = cv::Mat(target_rows,target_cols,largest_cone.type());

  if (window_man.is_configured==false) {
    window_man.configure(window_size);//largest_cone);//window_size
    window_man.set_window("c0",0);
    window_man.set_window("c1",1);
    window_man.set_window("c2",2);
    window_man.set_window("c3",3);
  }

  //assert (counter==2000);

  //for (auto i:cones){ imshow("fuck",i); cv::waitKey(0);}
  ...
  while(1){
    ...
    for (int i=0; i<cones.size(); i++){
      ...
      //cv::Mat resized_cone;
      //std::cout << window_size.size() << std::endl;

      //cv::resize(cone, resized_cone, window_size.size(), cv::INTER_LINEAR);

      std::string window_name = "c"+std::to_string(i);
      window_man.set_image(window_name, cone);
    }

    cv::Mat main_image = window_man.get_image();
